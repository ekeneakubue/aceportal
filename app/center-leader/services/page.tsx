...
export default function ServicesManagement() {
  ...
  const fetchServices = async () => {
    setLoading(true);
    try {
      const response = await fetch('/api/center-leader/services');
  ...
  const handleCreateService = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        showMessage('error', data.message || 'Failed to create service');
      }
    } catch (error) {
      showMessage('error', 'Error creating service');
      console.error('Error:', error);
    } finally {
      setLoading(false);
    }
  ...
  const handleUpdateService = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/center-leader/services/${selectedService.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...formData,

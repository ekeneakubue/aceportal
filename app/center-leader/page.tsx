...
import { 
  ...
  Clock, BarChart3, PieChart, ArrowUpRight, ArrowDownRight, Database
} from 'lucide-react';
import CenterLeaderLayout from './components/CenterLeaderLayout';

export default function CenterLeaderDashboard() {
  ...
  const fetchRecentUsers = async () => {
    try {
      const response = await fetch('/api/center-leader/users/recent');
      const data = await response.json();
      if (data.success) {
  ...
  const roleDistribution = [
    ...
    { role: 'Staff', count: stats.staff, color: 'bg-gray-500' },
  ];
